...
export function TaskBoard() {
  ...
  const handleDragEnd = (result: any) => {
    ...
    const updatedTask = {
      ...task,
      status: destination.droppableId as "available" | "in-progress" | "completed" | "approved",
  ...
  const handlePointRequest = (e: React.FormEvent) => {
    ...
    setPointRequest({ taskTitle: "", points: 0, description: "", evidence: "" })
  }
  ...
  const nonRecurringTasks = Object.fromEntries(
    Object.entries(tasks).map(([status, taskList]) => [status, taskList.filter((task) => !task.recurring)]),
  )
          ...
          <Dialog open={isRequestModalOpen} onOpenChange={setIsRequestModalOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                ...
                <DialogDescription>Submit a request for points for a completed task.</DialogDescription>
              </DialogHeader>
                ...
                <div className="grid gap-4 py-4">
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input
                      ...
                      onChange={(e) => setPointRequest({ ...pointRequest, taskTitle: e.target.value })}
                  ...
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input
                      ...
                      onChange={(e) => setPointRequest({ ...pointRequest, points: Number.parseInt(e.target.value) })}
                      className="col-span-3"
                    />
                  </div>
                  <div className="grid grid-cols-4 items-center gap-4">
          ...
          <DialogContent className="sm:max-w-[500px]">
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <div className="rounded-md bg-muted p-3 text-sm">{selectedTask?.description}</div>
              </div>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <Label>Status</Label>
                  <div className="font-medium capitalize">{selectedTask?.status.replace("-", " ")}</div>
                </div>
              </div>
              {selectedTask?.status === "completed" && (
                <div className="grid gap-2">
                  ...
                  <div className="flex gap-2">
                    <Input id="evidence" placeholder="Link to evidence or description" />
                    <Button size="icon" variant="outline">
                      <LinkIcon className="h-4 w-4" />
                    </Button>
                  </div>
                  <Textarea placeholder="Additional comments or context" className="mt-2" />
                </div>
              )}
            </div>
            <DialogFooter>
              {selectedTask?.status === "available" && <Button>Claim Task</Button>}
              {selectedTask?.status === "in-progress" && <Button>Mark as Completed</Button>}
              {selectedTask?.status === "completed" && <Button>Submit for Approval</Button>}
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </CardContent>
